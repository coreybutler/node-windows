module.exports = {
  ...
  generateXml: function(config)
  {
    ...
    multi('arguments',config.nodeOptions, ' ');
    xml.push({arguments:config.script.trim()});
    multi('arguments',config.wrapperArgs,' ');
    ...
    if (config.logOnAs) {
      // Validate if logOnAs is object
      let username = '';
      if (config.logOnAs && typeof config.logOnAs === 'object') {
        const domain = config.logOnAs.domain || 'NT AUTHORITY';
        const user = config.logOnAs.account || 'LocalSystem';
        username = `${domain}\\${user}`;
      } else if (typeof config.logOnAs === 'string') {
        username = config.logOnAs !== '' ? config.logOnAs : 'NT AUTHORITY\\LocalSystem';
      }
      const password = config.logOnAs.password || '';
      xml.push({
        serviceaccount: {
          username,
          password,
